create or replace procedure sales.raw_to_nf(filter_date string)
begin

  declare sql_string string;
  set sql_string = (select concat('insert into sales.sales_nf (invoice_id, branch, city, product_line, payment_type, member_status, gender, transaction_time, amount, unit_price, cost, rating) ',
  ' select r.Invoice_ID, b.branch_id, c.city_id, pl.product_line_id, pt.payment_type_id, ms.member_status_id, g.gender_id, datetime(concat(r.date, " ", r.time, ":00")), r.Quantity, r.Unit_price, r.cogs, r.Rating',
